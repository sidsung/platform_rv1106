...
int inference_model(rknn_app_ctx_t *app_ctx, object_detect_result_list_t *od_results)
{
    int ret = -1;
...
int rknn_app_init(rknn_app_ctx_t *app_ctx)
{
    ...
    printf("[%s %d] rknn_init\n", __func__, __LINE__);

    ret = rknn_init(&ctx, (void*)app_ctx->model_path, 0, 0, NULL);
    if (ret < 0)
    {
    ...
    for (uint32_t i = 0; i < io_num.n_input; ++i) {
        ...
        printf("input_attrs[%d].size_with_stride=%d\n", i, input_attrs[i].size_with_stride);
        app_ctx->input_mems[i] = rknn_create_mem(ctx, input_attrs[i].size_with_stride);

        // Set input tensor memory
        ret = rknn_set_io_mem(ctx, app_ctx->input_mems[i], &input_attrs[i]);
        if (ret < 0) {
